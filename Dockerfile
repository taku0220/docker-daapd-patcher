...
\
 ...
 find /tmp/source/forked-daapd_patch -maxdepth 1 -name "*.patch" \
 	-exec /bin/sh -c 'patch --verbose -p1 < {}' \; && \
 autoreconf -i -v && \
 ./configure \
	--build=$CBUILD \
	--enable-chromecast \
...
RUN \
 ...
 apk add --no-cache \
	...
	sqlite-libs && \
\
 echo -e "\n**** configuration changes avahi-daemon ****" && \
